...
func _ready() -> void:
	...
	for ln in layers:
		...
		var f1 := await _grab(b + Vector2(0.45 / 16.0, 0.0))
		var f2 := await _grab(b + Vector2(0.45 / 16.0, 0.0))   # same position again: the noise floor
		var line := "SHIM %-10s" % (ln if ln != "" else "base")
		for r in rects:
			line += "  %s=%.4f/%.4f" % [str(r.position), _diff(f0, f1, r), _diff(f1, f2, r)]
		print(line)
		if n:
			n.visible = true
	get_tree().quit()
